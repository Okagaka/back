...
public class TmapRouteMatrixService {
    ...
    public List<MatrixRouteInfoDTO> estimateOptimizedTravelTime(List<Coordinate> origins, List<Coordinate> destinations) {
        JsonNode matrixResponse = getMatrix(origins, destinations);

        List<MatrixRouteInfoDTO> routeInfoList = new ArrayList<>();
        ...
        if (matrixResponse.has("matrixRoutes")) {
            ...
            for (JsonNode route : matrixRoutes) {
                ...
                int duration = route.get("duration").asInt(); // 초 단위

                // origin/destination 좌표 포함
                Coordinate originCoord = origins.get(originIndex);
                Coordinate destCoord = destinations.get(destinationIndex);

                routeInfoList.add(new MatrixRouteInfoDTO(originIndex, destinationIndex, originCoord, destCoord, duration));
            }
        }
